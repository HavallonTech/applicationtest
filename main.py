...
class Btqueue(db.Model):  # Capitalize 'Btqueue' to follow naming conventions
    ...
    def __init__(self, pat_phone, pat_procedure, pat_id, queue_time, service_status, arrival_time, departure_time,  departmentid):
        ...
        self.departmentid = session['userdept']  # Ensure departmentid is initialized properly
...
def login():
    ...
    selected_option = request.form.get("userdept")  # Get value from form

    # handle attempt to login ad ICT admin
    

    user = appusers.query.filter_by(username=username).first()
    if user and user.check_password(userpass):
        session['username']=username
        session['userdept']=selected_option
        #if int(session['userdept']) == 1:
        return redirect(url_for("dashboard", username=session['username'], userdept=session['userdept']))
        #elif int(session['userdept']) == 2:
        #return redirect(url_for("carddashboard", username=session['username'], userdept=session['userdept']))
    else:
        return render_template("index.html")
...
def insert():
    if request.method == "POST":
        ...
        departmentid = session['userdept']
        new_patient = Btqueue(pat_phone=pat_phone, arrival_time=arrival_time , pat_procedure=pat_procedure, departure_time=departure_time , pat_id=pat_id, queue_time =queue_time, service_status=1, departmentid=departmentid )
...
@app.route('/dopplerview')
def dopplerview():
    doppler_data = Btqueue.query.filter_by(pat_procedure="Doppler", service_status=1).all()
    return render_template('dopplerview.html', doppler_data=doppler_data)

@app.route('/ecgview')
def ecgview():
    ecg_data = Btqueue.query.filter_by(pat_procedure="ECG", service_status=1).all()
    return render_template('ecgview.html', ecg_data=ecg_data)

@app.route('/echoview')
def echoview():
    echo_data = Btqueue.query.filter_by(pat_procedure="Echo", service_status=1).all()
    return render_template('echoview.html', echo_data=echo_data)
...
if __name__ =="__main__":
    with app.app_context():
        db.create_all()
    #with app.app_context():
    #    db.session.query(Btqueue).filter(Btqueue.departmentid == None).update({"departmentid": 1})
    #    db.session.commit()
